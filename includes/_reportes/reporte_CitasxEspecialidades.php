...
$pdf->SetAuthor('');
$pdf->SetTitle('Reporte de Citas Por Especialidad');
...
<h5> </h5>
<h1>Reporte de Citas por Especialidad</h1>
<h5> </h5>
...
// Consulta SQL para obtener los datos de citas por especialidad
$sql = "SELECT especialidad, 
               SUM(CASE WHEN tipo_paciente = 'Empleado' THEN 1 ELSE 0 END) AS empleados,
               SUM(CASE WHEN tipo_paciente = 'Beneficiario' THEN 1 ELSE 0 END) AS beneficiarios,
               COUNT(*) AS total
        FROM citas";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $html .= '<tr>';
        $html .= '<td class="bold mayus font-primary">' . $row['especialidad'] . '</td>';
        $html .= '<td class="text-center">' . $row['empleados'] . '</td>';
        $html .= '<td class="text-center">' . $row['beneficiarios'] . '</td>';
        $html .= '<td class="text-center">' . ($row['empleados'] + $row['beneficiarios']) . '</td>';

        // Calcular el total de la fila y sumarlo al total general
        $total = $row['empleados'] + $row['beneficiarios'];
        $totalGeneral += $total;
        
        // Sumar a los totales de cada columna
        $totalEmpleados += $row['empleados'];
        $totalBeneficiarios += $row['beneficiarios'];
    ...
    $html .= '<tr>';
    $html .= '<td colspan="4" class="text-center">No se encontraron citas entre las fechas seleccionadas.</td>';
    $html .= '</tr>';
}
...
$pdf->Output('reporte_citas_especialidad.pdf', 'I');
